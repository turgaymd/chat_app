...
MessageRouter.delete("/messages/:id", protect, async (req, res) => {
  ...
  if (messages.deletedCount>0) {
    res.status(200).json({ success: true, message: "Messages are deleted" });
  } else {
...
MessageRouter.post("/messages/:id",protect, upload.single("image"), asyncHandler(async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({ error: "Unable to send a message", details: err.message });
    }
  })
);
MessageRouter.delete("/messages/receiver/:id", protect, async (req, res) => {
  try {
    ...
    if (result.deletedCount > 0) {
      res.status(200).json({
        success: true,
        message: "Sent messages deleted successfully.",
      });
    } else {
